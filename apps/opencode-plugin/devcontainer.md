...
| Variable | Purpose |
|----------|---------|
| `PLANNOTATOR_REMOTE=1` | Tells Plannotator to skip the default browser opener and use a fixed remote port |
...
**Note:** Remote mode skips the default browser opener. In VS Code devcontainers, the `BROWSER` helper may still open the URL automatically; otherwise, manually navigate to the forwarded URL when you see the agent call `submit_plan`.
